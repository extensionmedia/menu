...
@section('content')
    ...
    <div class="border rounded-lg overflow-hidden my-4">
        <div class="bg-gray-100 py-2 px-4 text-gray-800 text-sm">
            <i class="fas fa-plus"></i> Modifier une category
            @php
                $folder = 'categories/'.$UID
            @endphp
        </div>
        <form method="POST" action="{{route('category.update', ['category'=>$category])}}" class="bg-white py-8 px-4">
            @csrf
            @method('PUT')
            ...
            <div class="text-gray-600 text-xs flex mb-20">
                ...
                <div class="relative">
                    ...
                    <input type="hidden" name="filename" id="filename" value="{{$category->image}}">
